...
export function QuestChat({ questId, isParticipant, previewOnly = false }: QuestChatProps) {
  ...
  return (
    <div className={`flex flex-col bg-gray-50 dark:bg-gray-800 rounded-3xl overflow-hidden border border-gray-100 dark:border-gray-700 transition-colors ${previewOnly ? '' : 'h-[500px]'}`}>
              ...
              return (
                ...
                >
                  ...
                  <div className={`max-w-[75%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                    {!isMe && <span className="text-[10px] text-gray-500 dark:text-gray-400 ml-1 mb-0.5">{msg.profiles?.display_name || msg.profiles?.username}</span>}
                    <div className={`px-4 py-2.5 rounded-2xl text-sm ${isMe ? 'bg-[#58CC02]/10 text-green-900 dark:text-green-300 rounded-br-sm' : 'bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-bl-sm border border-gray-100 dark:border-gray-600 shadow-sm'}`}>
                      {msg.body}
                    </div>
      ...
      {!previewOnly && (
        <form onSubmit={handleSend} className="p-3 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 flex gap-2 transition-colors">
          <input
            type="text"
            value={text}
            ...
            className="flex-1 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white border-0 rounded-full px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary disabled:opacity-50 placeholder-gray-400 dark:placeholder-gray-500 transition-colors"
          />
          <button
            type="submit"
