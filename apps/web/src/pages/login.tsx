...
export function Login() {
  ...
  return (
    <div className="login-page-wrapper relative w-full h-screen bg-[#000000] text-white font-sans overflow-hidden selection:bg-primary/20">
      
      {/* 3D R3F Canvas Container (Fixed in the background behind the scroll layers) */}
      ...
      {/* Soft coordinate space gridlines background - Fixed behind text */}
      <div className="fixed inset-0 bg-[radial-gradient(#EE6C1F08_1.5px,transparent_1.5px)] [background-size:24px_24px] pointer-events-none z-0" />

      {/* 3D Morphing Globe Scroll Timeline Controller */}
      <ScrollController progressRef={progressRef} />

      {/* Native CSS Scroll Snapping Foreground Container */}
      <div
        id="foreground-scroll-container"
        className="w-full h-screen overflow-y-auto snap-y snap-mandatory relative z-10 no-scrollbar"
      >
        {/* FOLD 1: HERO FOLD (Height is 100vh, snaps to start) */}
        <section
          id="hero-fold-section"
          className="w-full h-screen snap-start flex flex-col justify-between items-center py-12 px-6 text-center relative z-10"
        >
          ...
          <div 
            ...
            className="w-full max-w-lg mx-auto pointer-events-none mt-20 px-4"
          >
          ...
          <button
            ...
            onClick={() => {
              const container = document.getElementById('foreground-scroll-container')
              if (container) {
                container.scrollTo({
        ...
        {/* FOLD 2: LOGIN/AUTH FOLD (Height is 100vh, snaps to start) */}
        <section
          id="login-fold-section"
          className="w-full h-screen snap-start flex items-center justify-center px-6 relative z-10"
        >
          <div 
            id="auth-card-wrapper"
            className="max-w-md w-full pointer-events-auto"
          >
            <div className="w-full bg-[#120B06]/65 backdrop-blur-[32px] border border-white/10 shadow-[0_24px_50px_-12px_rgba(0,0,0,0.7)] p-8 sm:p-10 rounded-[28px] text-center flex flex-col gap-6">
              <div>
